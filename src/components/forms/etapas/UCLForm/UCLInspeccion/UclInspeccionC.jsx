...
import { INSPECCION_A_ITEMS_UCL, INSPECCION_C_ITEMS_UCL } from "../../../../../constants/INSPECCION_ITEMS_UCL";
...
const UclInspeccionC = () => {
  ...
  const { newEnsayoUcl  } = useEnsayoData();
  useEffect(() => {
    if (inspeccionActual) {
...
if (!ensayoExisteEnOt) {
  try {
    ...
    const resp = await newEnsayoUcl(data);
...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        ...
        console.log("No existe ensayoID...");
      }
    } catch (error) {
  ...
  return (
    <form className="recepcion-form" onSubmit={handleSubmit(onSubmit)}>
      <h3 className="form-title">Inspección UCL C</h3>
      ...
      <h3>Items</h3>
      {INSPECCION_C_ITEMS_UCL.sistemaHidraulicoPcpVh60.map((item) => (
